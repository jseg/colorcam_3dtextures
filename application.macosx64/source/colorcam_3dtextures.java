...
public class colorcam_3dtextures extends PApplet {
...
AniSequence seq_y;
AniSequence seq_d;
float s = 1.0f;  //zoom as a fraction of display height
float s_t = 1.0f;
  ...
  if(isOn == 1){
     //scaleOrg();
      ...
      for (int j=0; j<cols+1; j++){
         ...
         if(((abs(c.x)-(0.5f*s*tex.width))<(width/2))&&((abs(c.y)-(0.5f*s*tex.height))<(height/2))){
           pushMatrix();
  ...
  if (key == CODED) {
    if (keyCode == UP) {
     ...
     s_t *= .85f;
      ...
      slideTarget = slide*height;
      Ani.to(this,1.5f,"slidePos",slideTarget);
    }
...
public void origins(){
  ...
  for (int i = -rows/2; i<rows/2+1; i++){
    for (int j = -cols/2; j<cols/2+1; j++){
     org[index] = new PVector(i*1.0f,j*1.0f,0.0f); 
     //println(org[index]);
     //println(org.length);
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#000000", "--stop-color=#cccccc", "colorcam_3dtextures" };
    if (passedArgs != null) {
